...
public class ArrivalReportService {
    ...
    @Autowired
    private ArrivalConsignmentService arrivalConsignmentService;
    @Autowired
    ...
    @Autowired
    private ArrivalEquipmentService arrivalEquipmentService;
    ...
    public JSONResult save(String copMsgId, String msgType, Manifest manifest) throws Exception{
        ...
        arrivalReport.setArrivalReportId(copMsgId);
        arrivalReport.setMessageid(copMsgId);
        arrivalReport.setMessagetype(msgType);

        arrivalReport.setDeclarationofficeid(declaration.getDeclarationOfficeID());

        if(declaration.getUnloadingLocation()!=null){
            UnloadingLocation unloadingLocation = declaration.getUnloadingLocation();
            arrivalReport.setUnloadinglocationid(unloadingLocation.getID());
            arrivalReport.setArrivaldate(unloadingLocation.getArrivalDate());
        }

        arrivalReport.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());

        arrivalReport.setBordertransportmeansId(bordertransportmeansId);

        this.insert(arrivalReport);
        ...
        /*** 集装箱 start **/
        List<TransportEquipment> transportEquipmentList = declaration.getTransportEquipmentList();
        if(transportEquipmentList!=null){
            for(TransportEquipment transportEquipment:transportEquipmentList){
                ...
                //运抵报告与集装箱关联表
                ArrivalEquipment arrivalEquipment = new ArrivalEquipment();
                arrivalEquipment.setArrivalReportId(arrivalReport.getArrivalReportId());
                arrivalEquipment.setTransportequipmentId(transportequipmentEnt.getTransportequipmentId());
                arrivalEquipmentService.insert(arrivalEquipment);
        ...
        if(listConsignment!=null && listConsignment.size()>0){
            for(Consignment consignment:listConsignment){
                ...
                //保存关系表
                ArrivalConsignment arrivalConsignment = new ArrivalConsignment();
                arrivalConsignment.setConsignmentId(consignmentEntity.getConsignmentId());
                arrivalConsignment.setArrivalReportId(arrivalReport.getArrivalReportId());
                arrivalConsignmentService.insert(arrivalConsignment);
