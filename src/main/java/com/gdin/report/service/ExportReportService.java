...
public class ExportReportService {
    ...
    public JSONResult save(String copMsgId, String msgType, Manifest manifest) throws Exception{
        ...
        exportReport.setExportReportId(copMsgId);
        exportReport.setMessageid(copMsgId);
        exportReport.setMessagetype(msgType);
        exportReport.setRepresentativepersonname(declaration.getRepresentativePerson()==null?"":declaration.getRepresentativePerson().getName());
        exportReport.setExitcustomsofficeid(declaration.getExitCustomsOffice()==null?"":declaration.getExitCustomsOffice().getID());
        exportReport.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());
        exportReport.setAgentid(declaration.getAgent()==null?"":declaration.getAgent().getID());
        exportReport.setCarrierid(declaration.getCarrier()==null?"":declaration.getCarrier().getID());
        exportReport.setBordertransportmeansId(bordertransportmeansId);
        this.insert(exportReport);
        ...
        if(listConsignment!=null && listConsignment.size()>0){
            for(Consignment consignment:listConsignment){
                ...
                //保存关系表
                ExportConsignment exportConsignment = new ExportConsignment();
                exportConsignment.setConsignmentId(consignmentEntity.getConsignmentId());
                exportConsignment.setExportReportId(exportReport.getExportReportId());
                exportConsignmentService.insert(exportConsignment);
