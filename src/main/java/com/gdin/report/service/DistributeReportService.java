...
public class DistributeReportService {
    ...
    @Autowired
    private  DistributeConsignmentService distributeConsignmentService;
    @Autowired
    ...
    public JSONResult save(String copMsgId, String msgType, Manifest manifest) throws Exception{
        ...
        //报文主表
        DistributeReport distributeReport = new DistributeReport();
        distributeReport.setDistributeReportId(copMsgId);
        distributeReport.setMessageid(copMsgId);
        distributeReport.setMessagetype(msgType);
        /*distributeReport.setRepresentativepersonname(declaration.getRepresentativePerson()==null?"":declaration.getRepresentativePerson().getName());
        distributeReport.setAgentid(declaration.getAgent()==null?"":declaration.getAgent().getID());
        distributeReport.setExitcustomsofficeid(declaration.getExitCustomsOffice()==null?"":declaration.getExitCustomsOffice().getID());
        */

        distributeReport.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());
        //distributeReport.setCarrierid(declaration.getCarrier()==null?"":declaration.getCarrier().getID());
        distributeReport.setBordertransportmeansId(bordertransportmeansId);

        this.insert(distributeReport);
        ...
        if(listConsignment!=null && listConsignment.size()>0){
            for(Consignment consignment:listConsignment){
                ...
                //保存关系表
                DistributeConsignment distributeConsignment = new DistributeConsignment();
                distributeConsignment.setConsignmentId(consignmentEntity.getConsignmentId());
                distributeConsignment.setDistributeReportId(distributeReport.getDistributeReportId());
                distributeConsignmentService.insert(distributeConsignment);
                ...
                /*** 提运单的分拨分流运输工具 start**/
                List<BorderTransportMeans> borderTransportMeansList = consignment.getBorderTransportMeans();
                if(borderTransportMeansList!=null && borderTransportMeansList.size()>0){
                    for(BorderTransportMeans borderTransportMeansItem:borderTransportMeansList){
                        Bordertransportmeans entBordertransportmeans = new Bordertransportmeans();
                        entBordertransportmeans.setId(borderTransportMeansItem.getId());
                        entBordertransportmeans.setJourneyid(borderTransportMeansItem.getJourneyID());
                        entBordertransportmeans.setTypecode(borderTransportMeansItem.getTypeCode());
                        entBordertransportmeans.setName(borderTransportMeansItem.getName());
                        entBordertransportmeans.setFirstarrivallocationid(borderTransportMeansItem.getFirstArrivalLocationID());
                        entBordertransportmeans.setArrivaldatetime(borderTransportMeansItem.getArrivalDateTime());
                        entBordertransportmeans.setDeparturedatetime(borderTransportMeansItem.getDepartureDateTime());
                        entBordertransportmeans.setActualdatetime(borderTransportMeansItem.getActualDateTime());
                        entBordertransportmeans.setCompleteddatetime(borderTransportMeansItem.getCompletedDateTime());

                        entBordertransportmeans.setLoadinglocationid(borderTransportMeansItem.getLoadingLocation()==null?""
                                :borderTransportMeansItem.getLoadingLocation().getID());

                        entBordertransportmeans.setUnloadinglocationid(borderTransportMeansItem.getUnloadingLocation()==null?""
                                :borderTransportMeansItem.getUnloadingLocation().getID());

                        entBordertransportmeans.setFreetext(borderTransportMeansItem.getFreetext());

                        entBordertransportmeans.setUnloadingdatetime(borderTransportMeansItem.getUnloading()==null?""
                                :borderTransportMeansItem.getUnloading().getUnloadingDatetime());

                        entBordertransportmeans.setDespatchdatetime(borderTransportMeansItem.getDespatchdatetime());

                        entBordertransportmeans.setCargofacilitylocation(borderTransportMeansItem.getCargofacilitylocation());

                        bordertransportmeansService.insert(entBordertransportmeans);

                        //提运单的分拨分流运输工具 关系表
                        ConsignmentTransport consignmentTransport = new ConsignmentTransport();
                        consignmentTransport.setConsignmentId(consignmentEntity.getConsignmentId());
                        consignmentTransport.setBordertransportmeansId(entBordertransportmeans.getBordertransportmeansId());
                        consignmentTransportService.insert(consignmentTransport);
                    }
                }
