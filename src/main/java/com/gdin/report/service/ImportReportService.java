...
public class ImportReportService {
    @Autowired
    private ImportReportMapper importReportMapper;
    @Autowired
    ...
    public JSONResult save(String copMsgId,String msgType,Manifest manifest) throws Exception{
        ...
        importReport.setImportReportId(copMsgId);
        importReport.setMessageid(copMsgId);
        importReport.setMessagetype(msgType);
        importReport.setRepresentativepersonname(declaration.getRepresentativePerson()==null?"":declaration.getRepresentativePerson().getName());
        importReport.setExitcustomsofficeid(declaration.getExitCustomsOffice()==null?"":declaration.getExitCustomsOffice().getID());
        importReport.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());
        importReport.setAgentid(declaration.getAgent()==null?"":declaration.getAgent().getID());
        importReport.setCarrierid(declaration.getCarrier()==null?"":declaration.getCarrier().getID());
        importReport.setBordertransportmeansId(bordertransportmeansId);
        this.insert(importReport);
        ...
        if(listConsignment!=null && listConsignment.size()>0){
            for(Consignment consignment:listConsignment){
                ...
                //保存关系表
                ImportConsignment importConsignment = new ImportConsignment();
                importConsignment.setConsignmentId(consignmentEntity.getConsignmentId());
                importConsignment.setImportReportId(importReport.getImportReportId());
                importConsignmentService.insert(importConsignment);
