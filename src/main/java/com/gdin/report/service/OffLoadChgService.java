...
public class OffLoadChgService {
    ...
    @Autowired
    private OffLoadChgConsignmentService offLoadChgConsignmentService;
    ...
    public JSONResult save(String copMsgId, String msgType, Manifest manifest) throws Exception{
        ...
        //落装改配申请
        OffLoadChg offLoadChg= new OffLoadChg();
        offLoadChg.setOffLoadChgId(copMsgId);
        offLoadChg.setMessageid(copMsgId);
        offLoadChg.setMessagetype(msgType);
        offLoadChg.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());
        offLoadChg.setBordertransportmeansId(bordertransportmeansId);//变更前
        offLoadChg.setChangebordertransportmeansId(changebordertransportmeansId); //变更后
        this.insert(offLoadChg);
        ...
        if(listConsignment!=null && listConsignment.size()>0){
            for(Consignment consignment:listConsignment){
                ...
                //保存关系表
                OffLoadChgConsignment offLoadChgConsignment = new OffLoadChgConsignment();
                offLoadChgConsignment.setConsignmentId(consignmentOriginEntity.getConsignmentId());
                offLoadChgConsignment.setOffLoadChgId(offLoadChg.getOffLoadChgId());
                offLoadChgConsignmentService.insert(offLoadChgConsignment);
