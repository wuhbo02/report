...
public class TallyReportService {
    ...
    @Autowired
    private TallyConsignmentService tallyConsignmentService;
    @Autowired
    ...
    @Autowired
    private TallyEquipmentService tallyEquipmentService;
    ...
    public JSONResult save(String copMsgId, String msgType, Manifest manifest) throws Exception{
        ...
        tallyReport.setTallyReportId(copMsgId);
        tallyReport.setMessageid(copMsgId);
        tallyReport.setMessagetype(msgType);
        tallyReport.setDeclarationofficeid(declaration.getDeclarationOfficeID());
        //理货部门数据
        if(declaration.getTallyParty()!=null) {
            Party party = declaration.getTallyParty();
            tallyReport.setTallypartyid(party.getID());
            //理货责任人信息
            if(party.getContact()!=null){
                Contact contactEntiry = new Contact();
                contactEntiry.setName(party.getContact().getName());
                Contact contact = contactService.save(contactEntiry, party.getContact().getCommunication());
                tallyReport.setContactId(contact.getContactId());
            }
        }
        tallyReport.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());
        tallyReport.setBordertransportmeansId(bordertransportmeansId);
        this.insert(tallyReport);
        ...
        if(transportEquipmentList!=null){
            for(TransportEquipment transportEquipment:transportEquipmentList){
                ...
                tallyEquipment.setTallyReportId(tallyReport.getTallyReportId());
                tallyEquipment.setTransportequipmentId(transportequipmentEnt.getTransportequipmentId());
                tallyEquipmentService.insert(tallyEquipment);
        ...
        if(listConsignment!=null && listConsignment.size()>0){
            for(Consignment consignment:listConsignment){
                ...
                //保存关系表
                TallyConsignment tallyConsignment = new TallyConsignment();
                tallyConsignment.setConsignmentId(consignmentEntity.getConsignmentId());
                tallyConsignment.setTallyReportId(tallyReport.getTallyReportId());
                tallyConsignmentService.insert(tallyConsignment);
