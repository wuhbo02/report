...
public class DispatchReportService {
    ...
    public JSONResult save(String copMsgId, String msgType, Manifest manifest) throws Exception{
        ...
        dispatchReport.setDispatchReportId(copMsgId);
        dispatchReport.setMessageid(copMsgId);
        dispatchReport.setMessagetype(msgType);
        /*dispatchReport.setRepresentativepersonname(declaration.getRepresentativePerson()==null?"":declaration.getRepresentativePerson().getName());
        dispatchReport.setAgentid(declaration.getAgent()==null?"":declaration.getAgent().getID());
        dispatchReport.setExitcustomsofficeid(declaration.getExitCustomsOffice()==null?"":declaration.getExitCustomsOffice().getID());

        */
        dispatchReport.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());
        //dispatchReport.setCarrierid(declaration.getCarrier()==null?"":declaration.getCarrier().getID());
        dispatchReport.setBordertransportmeansId(bordertransportmeansId);

        this.insert(dispatchReport);
        ...
        if(listConsignment!=null && listConsignment.size()>0){
            for(Consignment consignment:listConsignment){
                ...
                //保存关系表
                DispatchConsignment dispatchConsignment = new DispatchConsignment();
                dispatchConsignment.setConsignmentId(consignmentEntity.getConsignmentId());
                dispatchConsignment.setDispatchReportId(dispatchReport.getDispatchReportId());
                dispatchConsignmentService.insert(dispatchConsignment);
