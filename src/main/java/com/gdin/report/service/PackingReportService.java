...
public class PackingReportService {
    ...
    @Autowired
    private PackingEquipmentService packingEquipmentService;
    @Autowired
    ...
    public JSONResult save(String copMsgId, String msgType, Manifest manifest) throws Exception{
        ...
        //报文主表
        PackingReport packingReport= new PackingReport();
        packingReport.setPackingReportId(copMsgId);
        packingReport.setMessageid(copMsgId);
        packingReport.setMessagetype(msgType);
        //申报地海关代码
        packingReport.setDeclarationofficeid(declaration.getDeclarationOfficeID());
        //装箱清单传输人
        packingReport.setRepresentativepersonname(declaration.getRepresentativePerson()==null?""
            :declaration.getRepresentativePerson().getName());
        //备注
        packingReport.setAdditionalinformation(declaration.getAdditionalInformation()==null?"":declaration.getAdditionalInformation().getContent());
        //运输工具数据
        packingReport.setBordertransportmeansId(bordertransportmeansId);
        this.insert(packingReport);
        ...
        if(transportEquipmentList!=null){
            for(TransportEquipment transportEquipment:transportEquipmentList){
                ...
                //报文与集装箱关联表
                PackingEquipment packingEquipment = new PackingEquipment();
                packingEquipment.setPackingReportId(packingReport.getPackingReportId());
                packingEquipment.setTransportequipmentId(transportequipmentEnt.getTransportequipmentId());
                packingEquipmentService.insert(packingEquipment);
                ...
                //提运单绑定了集装箱
                List<Consignment> listConsignment = transportEquipment.getConsignment();
                ...
                if(listConsignment!=null && listConsignment.size()>0){
                    for(Consignment consignment:listConsignment){
                        ...
                        //提运单与集装箱关系表
                        ConsignmentEquipments consignmentEquipments = new ConsignmentEquipments();
                        consignmentEquipments.setConsignmentId(consignmentEntity.getConsignmentId());
                        consignmentEquipments.setTransportequipmentId(transportequipmentEnt.getTransportequipmentId());
                        consignmentEquipmentsService.insert(consignmentEquipments);
